...
class ContextCompressor(ContextEngine):
    ...
    def __init__(
        ...
        # When False (legacy opt-out), insert a deterministic
    ...
    def _build_static_fallback_summary(
        ...
        dropped_user_context = (
            f"Latest dropped user message (background only, not active): {user_asks[-1]!r}"
            if user_asks
            else "No dropped user message recoverable from deterministic fallback."
        )
        active_task = (
            "None recoverable from deterministic fallback. This fallback contains "
            "no active user request and no actionable plan. Respond only to the "
            "latest user message after this marker; use dropped-turn details below "
...
## Goal
Recovered from a deterministic fallback because the LLM context summarizer was unavailable. This fallback contains no active user request; respond only to the latest user message after this marker and use current file/system state for exact details.{previous_summary_note}
...
## Pending User Asks
None recoverable from deterministic fallback. The latest live user message after this marker is authoritative.
